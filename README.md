...
Plataforma editorial completa sobre IA, AWS e DevOps — não só um blog, mas o motor de conteúdo de ponta a ponta: CMS próprio, pipeline de imagens, SEO/observabilidade de produção, e (em construção) um camada de geração e distribuição de conteúdo assistida por IA — resumos automáticos, repasse para LinkedIn, geração de imagens, versão em inglês e newsletter.

Monorepo serverless 100% AWS, infraestrutura como código, escrito e operado em parceria com IA (Claude) atuando como Staff Engineer autônomo.
...
| Imagens | Sharp.js (Lambda) | Pipeline automática: 6 variantes (AVIF/WebP × 3 breakpoints) + LQIP blur |
...
| CI/CD | GitHub Actions | Build → test → `terraform apply` → deploy, 100% automático em `develop` |
...
- GSIs do DynamoDB otimizadas (`ALL` → `INCLUDE`), contador atômico de posts publicados (evita scan duplicado em paginação).
...
A ordem abaixo segue a lógica de implementação técnica, não de prioridade de produto: agrupa primeiro tudo que reaproveita a mesma peça de infraestrutura (geração de texto via LLM), depois a próxima peça de infraestrutura nova (geração de imagem), e só por último a camada de distribuição — que consome o que as etapas anteriores produzem.
