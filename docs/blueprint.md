...
#### 2.9. Editor de Conteúdo (Rich Text) - [NOVO/MANDATÓRIO]
* **Tecnologia:** **Tiptap Editor (v2+)**. (Substituiu o Quill.js para permitir extensibilidade via ProseMirror).
* **Estratégia Sênior (Vue 3 + TS):** 1. **Computed Tipada:** O editor deve ser acessado no template via uma `computed` (ex: `editorInstance`) que realiza o cast `as Editor | undefined`. Isso evita erros de "Property chain/isActive does not exist" causados pela reatividade `ShallowRef`.
    2. **Guardas de Renderização:** Usar `v-if="editorInstance"` na Toolbar e no `EditorContent` para garantir que o componente só seja renderizado após a inicialização do editor, prevenindo falhas de inicialização (tela branca).
    3. **Sincronização Segura:** O `watch` do `modelValue` deve usar `editor.commands.setContent(value, { emitUpdate: false })` para evitar loops infinitos de atualização.
---
---

### 3. Modelo de Dados (O "Cérebro")

Definição das tabelas e índices no **Amazon DynamoDB**. (ATUALIZADO v1.3)

#### 3.1. Tabela 1: `Posts`
* **Propósito:** Armazena todo o conteúdo principal do blog.
* **Chave Primária (PK):** `slug` (string)
* **Diretriz de Integridade (MANDATÓRIO):** O campo `conteudo_html` deve ser validado no Backend para garantir que estruturas complexas (como os Nodes de Callout do Tiptap) contenham as classes CSS obrigatórias (`content-callout`, `callout-content-area`) para evitar quebras de layout no frontend.
* **Atributos:**
    * `titulo` (string)
    ...
    * `autor_id` (string)
    * `status` (string): ("Publicado", "Rascunho", "Programado")
...
* **GSI 1: `StatusPorData`**: PK: `status`, SK: `data_atualizacao`
* **GSI 2: `CategoriaPorData`**: PK: `categoria_slug`, SK: `data_atualizacao`
* **GSI 3: `ProjetoPorData`**: PK: `e_projeto`, SK: `data_publicacao` (Ascendente)
* **GSI 4: `PopularesPorData`**: PK: `e_popular`, SK: `data_atualizacao`
* **GSI 5: `StatusProgramadoPorData`**: PK: `status`, SK: `data_publicacao_programada`

#### 3.3. Tabela 2: `Autores`
* **Propósito:** Armazena dados do autor para E-E-A-T.
* **Chave Primária (PK):** `autor_id` (string)
* **Atributos:**
    * `nome_exibicao` (string)
    * `bio` (string): (Rich Text estruturado via Tiptap)
...
* **Chave Primária (PK):** `categoria_slug`
* **Atributos:**
    * `nome_exibicao` (string)
...
Define a interface de administração interna. (ATUALIZADO v1.3)
...
* **Telas (Vistas/Rotas do Vue Router):**
    ...
    2.  **Vista 2: `view-editor` (Formulário de Post):**
        * **Editor de Texto:** **Tiptap Editor (v2+)**. (Mandatório: Seguir as diretrizes de blindagem da Seção 2.9).
        * **Configuração de Nodes (MANDATÓRIO):** * **Node Callout:** Deve ser implementado com uma extensão customizada que utilize `contentElement: '.callout-content-area'` no `parseHTML`. Isso isola o conteúdo editável dos rótulos fixos ("Dica!", "Atenção!") e ícones, evitando a duplicação de texto durante o salvamento e carregamento.
            * **Estilo de Listas:** O CSS `scoped` do editor deve emular o Design System, utilizando pseudo-elementos (`::before`) para renderizar as bolinhas amarelas (`var(--aws-orange)`), garantindo que o marcador e o texto permaneçam alinhados na mesma linha.
        * **Campos:** `titulo`, `slug`, `conteudo_html` (via Tiptap), `resumo`, `categoria_slug` (select), `imagem_destaque_url` (input), `imagem_destaque_alt_text` (input - **Mandatório**), `meta_titulo_seo` (input - **Mandatório**), `meta_descricao_seo` (textarea - **Mandatório**), `e_popular` (check), `e_projeto` (check).
    ...
    4.  **Vista 4: `view-author-edit` (NOVO):**
        ...
        * Campos: `nome_exibicao`, `bio` (Rich Text via Tiptap), `foto_avatar_url`, `foto_avatar_alt_text` (**Mandatório**), `linkedin_url`, `github_url`, `instagram_url` (Opcional).
...
Define o processo de deploy automatizado. (ATUALIZADO v1.2)
...
* **Validação de Build (NOVO/MANDATÓRIO):** * O workflow de build do Admin (Vue 3) deve obrigatoriamente validar a integridade das extensões customizadas do Tiptap. 
    * Se o ficheiro `admin/src/components/Callout.ts` ou a lógica de tipagem do `RichTextEditor.vue` falharem no lint/build, o deploy deve ser abortado para evitar que o editor seja publicado com erros de inicialização (tela branca).
...
* **Infra Adicional (Otimização de Imagem):** O Terraform deve criar:
    ...
    * **Lógica da `ImageProcessorLambda`:**
