...
### Branch and PR (required, no exceptions for small changes)

Never commit directly on `develop` or `main`. Create a short-lived branch (`feature/*`, `fix/*`, `docs/*`, `chore/*`, `test/*`, `ci/*`) from `develop`, commit there, push the branch, then open a PR back to `develop` — even for a one-line change. Full policy: `docs/engineering/standards/git-and-review-workflow.md`.

```bash
git checkout -b fix/short-description
git commit -m "fix(scope): message"
git push -u origin fix/short-description
gh pr create --base develop --head fix/short-description --title "..." --body "..."
```

### After Merge

```bash
# Verify pipeline (merging into develop triggers CD)
gh run list
```
...
- [ ] Create a short-lived branch from `develop` before editing anything (never commit on `develop` directly)
...
- [ ] Push the branch, open a PR to `develop`, merge → monitor `gh run list`
...
5. Follow git workflow: branch from `develop` → `git commit` → push branch → PR → merge → monitor pipeline
