...
57. **Itens P1/P2 remanescentes da auditoria de qualidade de engenharia (2026-08-02)** — de `auditoria-qualidade-mgoncalves-editorial-platform.md` + `auditoria-confirmacao-expansao-2026-08-02.md` (achados já confirmados contra o código real) + `relatorio-auditoria-execucao-2026-08-02.md` (o que já foi corrigido nessa rodada: P0.1/P0.2/P0.3/P1.2, gates de segurança reais no CD, validação Zod, versionamento otimista completo no admin — ver os 3 arquivos pro histórico completo). Restam:
    ...
    - **P2.5 — reorganizar documentação** (`CLAUDE.md`/README grandes demais, misturando arquitetura durável com contexto de sessão) em `docs/architecture/`, `docs/operations/`, `docs/decisions/` (ADRs), etc. Decisão editorial de Marcelo (o que vira ADR, o que é só changelog), não correção técnica objetiva — não é do tipo que deve ser decidido e executado sozinho no mesmo ciclo de outros fixes. **Adiado de novo (2026-08-02, sessão do P1.1)** por recomendação explícita: fazer as duas mudanças estruturais (contratos compartilhados + reorg de docs) na mesma sessão dobraria o raio de risco sem necessidade — nenhuma depende da outra. Fica para uma sessão própria, com Marcelo decidindo o mapeamento seção-a-seção antes de mover qualquer conteúdo.
