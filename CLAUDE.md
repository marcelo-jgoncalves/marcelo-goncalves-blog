...
~~52. `/servicos` — decidir destino da página~~ ✅ **removida (2026-08-02, decisão de Marcelo)** — `frontend/app/servicos/` deletado; os pontos que apontavam pra rota (`HeaderNav` desktop/mobile, `Footer`, os 3 CTAs "Conhecer os serviços" de `o-projeto`/`todos-artigos`/`post/[slug]`) redirecionados pro anchor `/#servicos` já existente na Home institucional (mesmo padrão que `AdvisoryCta` já usava por default). `SERVICES_OVERVIEW_LINK` do dropdown removido — o trigger "Serviços" agora é o próprio link pro anchor, sem item redundante dentro do painel.
