...
32. **Upload real de imagem no admin (capa + inline) nunca foi testado ponta a ponta com Playwright** — sessões 39-41 validaram todo o resto da tela de escrita contra dados reais, mas upload exige um arquivo de teste real e mexe em S3 (`mediaApi.getUploadUrl` + presigned POST) — deixado de fora por tempo/risco. Ver `contexto/admin-test-plan.md` seção 3.2. **Reavaliado em 2026-08-02**: ainda de fora — mesmo risco/escopo de antes (precisa de fixture de imagem real + validar o presigned POST contra S3 de verdade, não só mockar); candidato a sessão própria junto do item #9. **Achado em 2026-08-02, não corrigido**: `frontend/scripts/create-test-post.mjs` já faz exatamente esse upload real (capa + inline, via `.file-input-hidden`, validando `naturalWidth`/`complete` da imagem publicada) — mas lê as credenciais de `docs/creds.txt`, caminho que não existe no repo (as credenciais reais estão em `contexto/creds.txt`). Não é claro se isso é um path desatualizado (script nunca rodou de fato desde uma reorganização de pasta) ou se `docs/creds.txt` existia e foi movido sem atualizar o script — precisa de investigação antes de mexer, não corrigido de passagem aqui pra não mascarar a causa real.
