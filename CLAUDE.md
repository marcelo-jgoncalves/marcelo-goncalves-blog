...
24. **~22-29 KiB de JavaScript não utilizado no bundle do frontend** — achado real do Lighthouse (`docs/auditoria-performance/03-perf-cwv.md`/relatórios brutos em `.lighthouseci/`): auditoria `unused-javascript` com score 0-50 em **7 das 8 execuções reais** (home, post, `/artigos`), consistente entre páginas — indica bundle compartilhado, não código específico de 1 rota. Afeta FCP (carregamento/parse de JS antes do primeiro paint). Investigar com `next build` + bundle analyzer (`@next/bundle-analyzer`) para identificar a origem exata antes de qualquer correção.
