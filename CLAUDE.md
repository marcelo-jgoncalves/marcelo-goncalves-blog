...
48. **Sem README por módulo nem `terraform-docs`, sem `tflint`/`checkov`** — achado na mesma análise: os 10 módulos em `infra/modules/` não têm documentação própria (só comentários inline), e o único scanner de qualidade Terraform no CI é o Trivy (segurança), sem lint de convenções/boas práticas. Maior escopo que os itens 46/47 — introduzir `tflint` pela primeira vez tende a revelar uma lista grande de achados pré-existentes de uma vez (mesmo padrão do que aconteceu ao ligar o Stylelint no CSS, §5). Tratar como sessão própria, não correção de passagem.
...
52. **`/servicos` — decidir destino da página** — a página existe no código (`frontend/app/servicos/page.tsx`, revisada visualmente na sessão 55) mas está fora do sitemap e da navegação (dropdown "Serviços" aponta direto pras 4 landings). Registro anterior de que teria sido "removida por completo" (sessão 47) ficou desatualizado quando ela foi retomada. Decidir: reintegrar (nav+sitemap) ou remover de fato.
53. **`EditorView.vue` (1143 linhas) e `RichTextEditor.vue` (1146)** — monólitos; extrair composables/subcomponentes **quando forem tocados por outro motivo**, não como migração retroativa (mesmo padrão da regra de CSS legado, §5).
...
38. **A11y — 4 achados pendentes da auditoria de 2026-06-30** (20/24 já corrigidos no commit `c9d99d4`): `page.tsx` (Home) tem div decorativo antes de heading nos cards "mais lidos" (refactor estrutural); `artigos/page.tsx` tem destaque com 2 links focáveis pro mesmo post (refactor estrutural) e `div.art-empty` sempre no DOM (sem impacto real); `ConsentModal` não devolve foco ao elemento que abriu o modal ao fechar (exige ref drilling via `ConsentManager`).
39. **Lighthouse — medir TBT pós-fixes de 2026-06-30** — 3 fixes aplicados (`f1776c6`/`ea9ddf2`/`21d9654`: ReadingProgressBar fora do Header, Header virou Server Component, 2ª instância de `Inter()` removida) pra reduzir TBT de 341ms, mas o delta real nunca foi remedido.
...
42. **Ajuste 17A (navegação global) — implementado 2026-07-27 com escopo reduzido**, pendências a tratar numa rodada dedicada:
    - drawer mobile sem semântica modal completa (falta `aria-modal`, focus trap real, overlay/portal dedicado — hoje é o painel não-modal já existente + scroll-lock);
    - analytics de navegação (`navigation_primary_click` etc., §37 do ajuste) não instrumentado — falta helper de analytics para plugar;
    - configuração de navegação não extraída pra módulo tipado próprio (`lib/navigation.ts`) — hoje os arrays em `HeaderNav.tsx` já são fonte única pra desktop+mobile, mas não isolados;
    - validação formal não executada: zoom 200%/400%, leitores de tela (NVDA/VoiceOver/TalkBack), navegação sem JavaScript, varredura de todos os breakpoints listados no ajuste (só desktop e 390px foram validados manualmente);
    - z-index não formalizado em escala explícita (header/overlay/drawer).
43. **Ajuste 17B (rodapé global) — implementado 2026-07-27 com escopo reduzido**, pendências a tratar numa rodada dedicada:
    - identidade visual do rodapé mantida no tema escuro petróleo/clay já em produção (`--petrol-deep`), não na paleta azul clara que o documento do ajuste assume como fallback (`#e0ecf7` etc.) — decisão deliberada: o dark petrol já É a identidade atual do site (Header, TOC, AuthorBox, NewsletterCTA usam o mesmo padrão), a paleta clara do ajuste é herança de uma versão anterior do design system;
    - `mailto:contato@marcelogoncalves.com` exibido sem confirmar se a caixa está operacional (SPF/DKIM/DMARC configurados, monitorada, testada em envio/resposta — condição do próprio ajuste §10.4). Mesmo endereço já estava hardcoded em `/contato` antes desta rodada;
    - analytics do rodapé (`footer_service_click` etc., §25 do ajuste) não instrumentado;
    - "Voltar ao topo" continua com o comportamento JS pré-existente (`window.scrollTo({behavior:'smooth'})`), não o link nativo `href="#top"` preferido pelo ajuste — não há `id="top"` no layout;
    - validação formal não executada: contraste AA medido, impressão (só a regra `display:none` foi adicionada, sem testar página impressa real), zoom 200%, leitores de tela, breakpoints completos (só desktop 1440px e mobile 390px foram validados manualmente).
